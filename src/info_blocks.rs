...
impl<'a> InfoBlockBuilder<'a> {
    ...
    fn add_to_first_finder(&mut self) {
        ...
        for column in 0..9 {
            ...
            self.qr_matrix
                .set_module((8, column), Module::Function(format_bitvec[bitvec_index]));
            bitvec_index += 1;
        }
        ...
        for row in (0..8).rev() {
            ...
            self.qr_matrix
                .set_module((row, 8), Module::Function(format_bitvec[bitvec_index]));
            bitvec_index += 1;
        }
    }
    ...
    fn add_to_other_finders(&mut self) {
        ...
        for row in 0..8 {
            if row == 7 {
                self.qr_matrix
                    .set_module((21 - 8, 8), Module::Function(true));
                break;
            }

            self.qr_matrix
                .set_module((20 - row, 8), Module::Function(format_bitvec[bitvec_index]));
            bitvec_index += 1;
        }

        for column in (0..8).rev() {
            self.qr_matrix.set_module(
                (8, 20 - column),
                Module::Function(format_bitvec[bitvec_index]),
            );
            bitvec_index += 1;
        }
    }
