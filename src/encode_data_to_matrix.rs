...
use crate::{module::Module, qr_matrix::QrMatrix, zig_zag_it::ZigZagIt};
...
impl<'a> DataEncoder<'a> {
    ...
    pub fn encode_to_matrix(&mut self) {
        ...
        for (row_cordinate, column_cordinate) in zigzag {
            ...
            if bitvector_index > 207 {
                break;
            }

            self.qr_matrix.set_module(
                (row_cordinate, column_cordinate),
                Module::Data(self.data[bitvector_index]),
            );
            bitvector_index += 1;
        }
    }
